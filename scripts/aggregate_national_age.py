"""
aggregate_national_age.py.
---------------------------
Aggregates the national occupation x age x sex employment table (from
pull_national_age.py) to all four SSYK2012 levels, mirroring aggregate.py's
regional aggregation but keyed by age instead of county.
"""
...
def default_paths(root: Path | None = None) -> Paths:
    ...
    return Paths(
        root=root,
        in_file=data_dir / "scb_national_age.parquet",
        map_file=root / "structure_ssyk12.csv",
        out_file=processed / "ssyk12_national_age_aggregated_ssyk4_to_ssyk1.parquet",
    )
...
def load_scb(paths: Paths) -> pl.DataFrame:
    """Load and normalise the national age employment parquet."""
    df = pl.read_parquet(paths.in_file)

    required = {"code_4", "age", "sex", "year", "value"}
...
def agg_level(lf: pl.LazyFrame, level_col: str, level_name: str) -> pl.LazyFrame:
    """Aggregate value to a single SSYK level, keeping age/demographic dims."""
    return (
...
def main() -> None:
    ...
    df = load_scb(paths)
    log.info("Loaded %d rows from scb_national_age", df.height)
